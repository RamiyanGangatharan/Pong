...
public class PaddleController : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        if ((isCollidingWithTopWall && move.y > 0) || (isCollidingWithBottomWall && move.y < 0)) 
        { 
            move = Vector3.zero; 
        }
    ...
    { 
        ...
        { 
            SetWallCollisionState(collision, true); 
        } 
    }
    ...
    { 
        if (collision.collider.CompareTag("Wall")) 
        { 
            SetWallCollisionState(collision, false); 
        } 
    }
    ...
    private void SetWallCollisionState(Collision2D collision, bool isColliding)
    {
        ...
        { 
            isCollidingWithBottomWall = isColliding; 
        }
        else if (collision.contacts[0].normal.y < 0) 
        { 
            isCollidingWithTopWall = isColliding; 
        }
    }
}
